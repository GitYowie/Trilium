(function () {
    ...
    function openJsonArrayGridEditor(initialValue, onSave, subgridConfig) {
        ...
        let cols = inferSubgridColumns(rows, subgridConfig);
        ...
        function renderGrid() {
            ...
            cols = inferSubgridColumns(rows, subgridConfig);
            ...
            act.style.borderBottom = '1px solid #bbb';
            act.style.padding = '4px';
            hr.appendChild(act);
            ...
            rows.forEach(function (row, ri) {
                ...
                cols.forEach(function (c) {
                    const colDefRaw = subgridConfig && Array.isArray(subgridConfig.columns)
                        ? subgridConfig.columns.find(function (d) { return (typeof d === 'string' ? d : d.key) === c; })
                        : null;
                    const colDef = typeof colDefRaw === 'string' ? { key: colDefRaw, type: 'string' } : (colDefRaw || { key: c, type: 'string' });
        ...
        toolbar.appendChild(mkBtn('Add row', function () {
            const row = {};
            cols.forEach(function (c) {
                const rawDef = subgridConfig && Array.isArray(subgridConfig.columns)
                    ? subgridConfig.columns.find(function (d) { return (typeof d === 'string' ? d : d.key) === c; })
                    : null;
                const def = typeof rawDef === 'string' ? { key: rawDef, type: 'string' } : rawDef;
                row[c] = defaultValueForType(def && def.type ? def.type : 'string');
            });
            rows.push(row);
            renderGrid();
        }));
        ...
        toolbar.appendChild(mkBtn('Save subgrid', function () {
            if (subgridConfig && Array.isArray(subgridConfig.columns)) {
                for (let ri = 0; ri < rows.length; ri++) {
                    for (const colDefRaw of subgridConfig.columns) {
                        const colDef = typeof colDefRaw === 'string' ? { key: colDefRaw, type: 'string', required: false } : colDefRaw;
                        if (!colDef || !colDef.key) continue;
                        if (colDef.required) {
    ...
    function createCellEditor(value, type, onChange, errorMessage, focusKey, subgridConfig) {
        ...
        if (type === 'boolean') {
            ...
            function refreshJsonArraySummary(rawValue) {
                ...
                const parsed = typeof rawValue === 'string' ? parseTypeValue(rawValue, 'json-array') : rawValue;
            ...
            if (type === 'json-array') {
                ...
                gridBtn.onclick = function () {
                    ...
                    const parsed = trimmed === '' ? [] : parseTypeValue(input.value, 'json-array');
                    if (!Array.isArray(parsed)) {
                        window.alert('Current value is not a valid JSON array. Fix JSON first.');
                        return;
                    }
                    openJsonArrayGridEditor(parsed, function (nextArray) {
                        input.value = JSON.stringify(nextArray);
                        onChange(nextArray);
                        refreshJsonArraySummary(nextArray);
                    }, subgridConfig);
                };
